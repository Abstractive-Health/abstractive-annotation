...
const AdminPage = () => {
  ...
  const jsonToCSV = (json, questions, finished, userFileNames) => {
    const rows = Object.keys(json).filter((fileName) =>
  ...
  const getUsers = async () => {
    try {
      const response = await fetch(
        `${process.env.API_HOST}/api/AllUsername?session_token=${session_token}`
      );
      const data = await response.json();
      setUsers(data);
    } catch (error) {
  ...
  const getAllFileNames = async () => {
    try {
      const response = await fetch(`${process.env.API_HOST}/api/fileNames`);
      const data = await response.json();
      setFileNames(data);
    } catch (error) {
      console.log("Error:", error);
  ...
  const getFileNamesByUser = async () => {
    try {
      const response = await fetch(
        `${process.env.API_HOST}/api/AllowedFiles?username=${selectedUser}`,
        {
          method: "GET",
          headers: {
  ...
  const handleCheckAll = async () => {
    ...
    for (const fileName of fileNames) {
      ...
      const response = await fetch(`${process.env.API_HOST}/api/addAllowedFile`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    ...
    <div className="customize">
      ...
      <div>
        <label htmlFor="userSelect" style={{ color: "white" }}>
          Select User:{" "}
        </label>
        <select
          id="userSelect"
